...
add_flags(parser)
parser.add_argument(
    "--data_dir", help="directory including noisy.json and clean.json files"
)
parser.add_argument(
    "--matching", default="sort", help="set this to dns for the dns dataset."
)
parser.add_argument(
    "--no_pesq",
    action="store_false",
...
def evaluate(args, model=None, data_loader=None):
    ...
    if data_loader is None:
        dataset = NoisyCleanSet(
            args.data_dir, matching=args.matching, sample_rate=args.sample_rate
        )
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(iterator):
                ...
                if args.device == "cpu":
                    pendings.append(
                        pool.submit(
                            _estimate_and_run_metrics, clean, model, noisy, args
                        )
                    )
                else:
                    ...
                    clean = clean.cpu()
                    pendings.append(pool.submit(_run_metrics, clean, estimate, args))
